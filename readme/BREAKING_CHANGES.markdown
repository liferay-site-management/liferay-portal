...
The `CTSQLModeThreadLocal` class was moved from the `portal-impl` module into the `portal-kernel` module. Additionally, its package was changed from `com.liferay.portal.change.tracking.sql` to `com.liferay.portal.kernel.change.tracking` to be consistent with the `portal-kernel` module's package naming scheme for the CT-related classes.
