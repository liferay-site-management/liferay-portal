...
public class DetectOrphanPagesCrawler {
	...
	public void detect(
		...
		for (int i = 0; (hitsJSONArray != null) && (i < hitsJSONArray.length());
			...
			for (int j = 0;
				 (linksJSONArray != null) && (j < linksJSONArray.length());
				 j++) {

				String link = linksJSONArray.optString(j, null);
		...
		Set<String> orphans = new LinkedHashSet<>(
			indexedByNormalizedURL.keySet());

		orphans.removeAll(linkedNormalizedURLs);
		orphans.remove(_normalize(seedURL));
