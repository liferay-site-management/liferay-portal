...
test(
	'Assert that pagination is working for My Sites widget',
	{
		tag: '@LPD-66998',
	},
	async ({apiHelpers, page, site}) => {
		...
		try {
			...
			await page
				.getByRole('option', {name: '40 Entries per Page'})
				.click();
...
test(
	'Assert joining sites with different membership types using My Sites portlet',
	{
		tag: '@LPD-66998',
	},
	async ({apiHelpers, page}) => {
		...
		try {
			...
			await page
				.getByRole('menuitem', {name: 'Request Membership'})
				.click();
...
test(
	'Assert My Sites widget displays user memberships assigned in multiple ways',
	{
		tag: '@LPD-66998',
	},
	async ({
		...
		try {
			...
			await page
				...
				.getByRole('link', {exact: true, name: newSite.name})
				.click();
			...
			await expect(page.getByText(newSite.name)).toBeVisible();
...
test(
	'Assert My Sites widget search shows active sites only',
	{
		tag: '@LPD-66998',
	},
	async ({apiHelpers, page, site}) => {
...
test(
	'Assert a user cannot join or leave a site with manual membership management disabled in My Sites widget',
	{
		tag: '@LPD-66998',
	},
