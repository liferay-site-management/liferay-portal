...
test('Could edit friendly URL and access by it', async ({
	...
	try {
		...
		// Assert that the redirection to the site's private page works using the new site's friendly URL

		await widgetPagePage.goto(privateLayout, newSiteFriendlyURL);
...
test('View public page via virtual host URL', async ({
	...
	try {
		...
		await context.route('**/*', (route) => {
			...
			if (url.includes(VIRTUAL_HOST_NAME)) {
				const newUrl = url.replace(
					`http://${VIRTUAL_HOST_NAME}:8080`,
					'http://localhost:8080'
				);
		...
		const virtualHostPage = await context.newPage();

		await virtualHostPage.goto(
			`http://${VIRTUAL_HOST_NAME}:8080/web${site.friendlyUrlPath}${layout.friendlyURL}`
		);

		await expect(
			virtualHostPage.getByRole('menuitem', {
