...
<div class="task-action">
	<aui:form action="<%= assignURL %>" method="post" name="assignFm">
		<div class="modal-body task-action-content">
			...
			<c:choose>
				<c:when test='<%= eventName.equals("assignToMe") %>'>
			...
			<aui:input cols="55" cssClass="task-action-comment" disabled='<%= !hasAssignableUsers && eventName.equals("assignTo") %>' name="comment" placeholder="comment" rows="1" type="textarea" />
		</div>
		...
		<div class="modal-footer">
			<div class="modal-item-last">
				<div class="btn-group">
					...
					<div class="btn-group-item">
						<aui:button disabled='<%= !hasAssignableUsers && eventName.equals("assignTo") %>' name="done" primary="<%= true %>" value="done" />
					</div>
				</div>
			</div>
		</div>
