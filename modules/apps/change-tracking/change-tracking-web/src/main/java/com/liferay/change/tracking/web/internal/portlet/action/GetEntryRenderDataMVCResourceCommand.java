...
public class GetEntryRenderDataMVCResourceCommand
	...
	private <T extends BaseModel<T>> JSONObject _getCTEntryRenderDataJSONObject(
		...
		if (!workflowData.isEmpty()) {
			workflowView = _getWorkflowView(themeDisplay, workflowData);
		}
	...
	private <T extends BaseModel<T>> Map<String, String> _getWorkflowData(
		...
		workflowData.put(
			"create-date", dateTimeFormat.format(workflowTask.getCreateDate()));

		if (workflowTask.getDueDate() != null) {
			workflowData.put(
				"due-date", dateTimeFormat.format(workflowTask.getDueDate()));
		}
		else {
			workflowData.put(
	...
	private String _getWorkflowView(
