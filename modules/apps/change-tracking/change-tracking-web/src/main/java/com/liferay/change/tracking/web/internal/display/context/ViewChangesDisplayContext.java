...
public class ViewChangesDisplayContext {
	...
	private Map<Long, List<Long>> _getRootPKsMap(CTClosure ctClosure) {
		if ((_modelClassNameId > 0) && (_modelClassPK > 0)) {
			Map<Long, List<Long>> rootPKsMap = ctClosure.getChildPKsMap(
				_modelClassNameId, _modelClassPK);

			for (long classNameId :
					CTClosureUtil.getParentClassNameIds(
						ctClosure, _modelClassNameId)) {

				rootPKsMap.put(
					classNameId,
					new ArrayList<>(
						CTClosureUtil.getPKs(ctClosure, classNameId)));
			}

			List<Long> primaryKeys = rootPKsMap.computeIfAbsent(
				_modelClassNameId, key -> new ArrayList<>());

			primaryKeys.add(_modelClassPK);
