...
public class CTSampleTestUtil {
	...
	private static void _addCTEntry(
		...
		preparedStatement.setLong(
			2, CounterLocalServiceUtil.increment(CTEntry.class.getName()));
		...
		preparedStatement.setLong(7, modelClassNameId);
		preparedStatement.setLong(8, modelClassPK);
		preparedStatement.setLong(9, 1);
		preparedStatement.setInt(10, changeType);

		preparedStatement.addBatch();
	}

	private static final String _INSERT_CT_ENTRY_SQL = StringBundler.concat(
		"insert into CTEntry (ctCollectionId, ctEntryId, companyId, userId, ",
		"createDate, modifiedDate, modelClassNameId, modelClassPK, ",
		"modelMvccVersion, changeType) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
