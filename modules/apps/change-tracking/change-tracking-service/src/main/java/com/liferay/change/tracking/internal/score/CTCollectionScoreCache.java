...
public class CTCollectionScoreCache {
	...
	public String getScoreSizeClassification(int score) {
		if (score > _THRESHOLD_LARGE) {
			return CTConstants.SCORE_SIZE_CLASSIFICATION_LARGE;
		}
		else if (score > _THRESHOLD_MEDIUM) {
	...
	private void _updateScore(long ctCollectionId, int delta) {
		...
		synchronized (lock) {
			...
			if (cachedScore == null) {
				...
				// The on demand calculation already reflects the CTEntry
				// that triggered this update, so no delta is added on top
				// of it.
	...
	private static final int _THRESHOLD_LARGE = 40000;

	private static final int _THRESHOLD_MEDIUM = 15000;
