...
<#list dataFactory.getSequence(dataFactory.maxPublicationJournalArticlePageCount) as publicationJournalArticlePageCount>
	...
	ctCollectionLayoutModel = dataFactory.newCTCollectionLayoutModel(ctCollectionId, groupId, groupId + "_ct_journal_article_" + publicationJournalArticlePageCount, "", dataFactory.getJournalArticleLayoutColumn(portletIdPrefix))
	/>

	${csvFileWriter.write("ct_layout", virtualHostModel.hostname + "," + groupModel.friendlyURL + "," + ctCollectionLayoutModel.friendlyURL + "\n")}

	<@insertLayout _layoutModel=ctCollectionLayoutModel />

	<#list dataFactory.getSequence(dataFactory.maxPublicationJournalArticleCount) as publicationJournalArticleCount>
		<#assign ctCollectionJournalArticleResourceModel = dataFactory.newCTCollectionJournalArticleResourceModel(ctCollectionId, groupId) />

		${dataFactory.toInsertSQL(ctCollectionJournalArticleResourceModel)}
		${dataFactory.toInsertSQL(dataFactory.newCTEntryModel(ctCollectionId, ctCollectionJournalArticleResourceModel, dataFactory.ctChangeTypeAddition))}
