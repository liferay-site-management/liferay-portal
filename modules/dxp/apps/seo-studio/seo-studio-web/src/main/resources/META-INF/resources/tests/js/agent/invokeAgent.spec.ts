...
describe('invokeAgent', () => {
	...
	it('subscribes, posts the agent instance, and resolves with the agent response', async () => {
		...
		const promise = invokeAgent({
			...
			context: {pageContent: 'Espresso machines for home brewing'},
		});
		...
		fakeEventSource.emit(
			...
			JSON.stringify({data: 'Best Espresso Machines for Home Brewing'})
		);

		await expect(promise).resolves.toBe(
			'Best Espresso Machines for Home Brewing'
		);
